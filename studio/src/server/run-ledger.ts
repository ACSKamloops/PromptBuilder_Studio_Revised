...
export function recordRun(entry: ProviderRunResult): RunRecord {
  ...
  if (ledger.history.length > MAX_ENTRIES) ledger.history.pop();

  recentRuns.unshift(recorded);
  if (recentRuns.length > MAX_ENTRIES) recentRuns.pop();

  return recorded;
}

export function listRuns(limit = MAX_ENTRIES): RunRecord[] {
  return recentRuns.slice(0, limit).map(cloneRunRecord);
}

export function listPromptRuns(promptId: string, limit = MAX_ENTRIES): RunRecord[] {
...
function prepareRunRecord(entry: ProviderRunResult, summary: PromptMetricsSummary): RunRecord {
  ...
  const metricsClone = entry.metrics
    ? {
        perNode: entry.metrics.perNode.map((node) => ({ ...node })),
        totals: { ...entry.metrics.totals },
      }
    : undefined;
  const benchmarksClone = entry.benchmarks
    ? {
        ssr: { ...entry.benchmarks.ssr },
        verificationEfficacy: { ...entry.benchmarks.verificationEfficacy },
      }
    : undefined;
  return {
    ...entry,
    ...
    benchmarks: benchmarksClone,
  };
}

function cloneRunRecord(record: RunRecord): RunRecord {
  const clone = prepareRunRecord(record, record.promptMetrics ?? buildSummaryFromRecord(record));
  return clone;
}

function buildSummaryFromRecord(record: RunRecord): PromptMetricsSummary {
  return {
    promptId: record.manifest.flow.id,
    promptName: record.manifest.flow.name,
    runCount: 1,
    lastRunId: record.runId,
    lastUpdated: record.completedAt,
    latency: {
      count: 1,
      mean: record.latencyMs,
      variance: 0,
      standardDeviation: 0,
      min: record.latencyMs,
      max: record.latencyMs,
      last: record.latencyMs,
    },
    tokens: {
      prompt: {
        count: 1,
        mean: record.usage.promptTokens,
        variance: 0,
        standardDeviation: 0,
        min: record.usage.promptTokens,
        max: record.usage.promptTokens,
        last: record.usage.promptTokens,
      },
      completion: {
        count: 1,
        mean: record.usage.completionTokens,
        variance: 0,
        standardDeviation: 0,
        min: record.usage.completionTokens,
        max: record.usage.completionTokens,
        last: record.usage.completionTokens,
      },
      total: {
        count: 1,
        mean: record.usage.totalTokens,
        variance: 0,
        standardDeviation: 0,
        min: record.usage.totalTokens,
        max: record.usage.totalTokens,
        last: record.usage.totalTokens,
      },
    },
  };
}
